...
This is a dynamic programming solution to an np complete problem that runs in exponential time
*/
#include<iostream>
#include<fstream>
#include<math.h>
...
void readGraph(vector<node>& graph)
{
	...
	//currentCity.x = 0;
	//currentCity.y = 0;
	//graph.push_back(currentCity);//dummy value, index starts from 1
	for (int i = 0; i < numCities; i++)
	{
		input >> currentCity.x;
		input >> currentCity.y;
...
double minTSPcost(vector<node>& graph)
{
	...
	{
		currentCost = costs2.find(allNodeValue + j * pow(2, 25))->second +
			(sqrt((graph[j].x - graph[0].x)*(graph[j].x - graph[0].x) + (graph[j].y - graph[0].y)*(graph[j].y - graph[0].y))); //eucledian distnace
